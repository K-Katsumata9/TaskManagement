# 機能要件

[← 要件定義書に戻る](requirements.md)

## 1. 機能要件（最小構成）

### 1.1 リスト（列）
- リストを作成できる（例：未着手／作業中／完了）
- リストを削除できる
- リストの内容を編集できる（タイトル）

### 1.2 カード（タスク）
- カードを作成できる（タイトル、優先順位を設定）
- カードの内容を編集できる（タイトル・優先順位・詳細・期限）
- カードには詳細（説明文）を設定できる（任意項目、タイトルだけでは書ききれない補足内容を記録する）
- カードには期限（日付）を設定できる（任意項目、カレンダー形式のモーダルUIから入力する）
- カードを削除できる
- カードをドラッグ&ドロップで別のリストに移動できる
- カードには作成日を自動で記録する

### 1.3 並び替え（ソート）
- カードは常にドラッグ&ドロップでリスト内の任意の位置に並び替えられる（基本の操作方法）
- 各リストに「優先順位で並び替え」「作成日で並び替え」ボタンを用意し、押した時点の一度だけ、その基準（優先順位の高い順・作成日の新しい順）でカードを並び替える
- ボタンによる並び替えは一時的な整列であり、その後は通常通りドラッグ&ドロップで自由に並び替えられる（並び替え方法が固定されるわけではない）
- 並び替えはリストごとに個別に行い、他のリストには影響しない

## 2. 操作の流れ

典型的な利用シーンにおける操作の流れは以下の通り。

1. ボード画面を開く（アプリ起動時に表示される唯一の画面）
2. 「＋リストを追加」から、リスト（例：未着手／作業中／完了）を作成する
3. 各リストに「＋カードを追加」からタスクを登録する
4. タスクの状況が変わったら、該当のカードを別のリストへドラッグ&ドロップで移動する
5. タスクの内容を変更したい場合はカードをクリックして編集、不要になったら削除する

```
[開始] → リスト作成 → カード作成 → （進捗に応じて）カードを別リストへ移動 → カード編集／削除
```

## 3. 対象外（今回は作らない機能）
- ユーザー登録・ログイン
- カードへのラベル・添付ファイル
- 複数ボードの管理
- コメント機能

## 4. 受け入れ基準（完成の判断基準）
以下がすべて満たされた状態をもって、第一版の完成とする。
- リストの作成・削除ができること
- カードの作成・編集・削除ができること
- カードをドラッグ&ドロップで別のリストへ移動でき、移動後も状態が保持されること（画面を再読み込みしても内容が消えないこと）
- PCの主要ブラウザ（Google Chrome最新版）で問題なく表示・操作できること
